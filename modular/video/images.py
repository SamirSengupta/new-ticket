...
def _find_gemini_image_gen() -> str | None:
    ...
    # The coleslaw package lives at: .../coleslaw_versions/coleslaw/video/
    # v-content-creator lives at:         .../coleslaw_versions/v-content-creator/
    this_dir     = os.path.dirname(os.path.abspath(__file__))
    pkg_root     = os.path.dirname(this_dir)          # .../coleslaw
    versions_dir = os.path.dirname(pkg_root)           # .../coleslaw_versions
...
def _wikimedia_search(keywords: str, story_context: str = "") -> str | None:
    ...
    try:
        req = urllib.request.Request(api, headers={'User-Agent': 'ColeSlaw/1.0'})
...
def _unsplash_search(keywords: str, width: int, height: int, access_key: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'ColeSlaw/1.0'})
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read())
