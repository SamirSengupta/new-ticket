...
class TestReplVoiceIntegration:
    def test_voice_in_commands(self):
        import coleslaw
        assert "voice" in coleslaw.COMMANDS

    def test_voice_command_callable(self):
        import coleslaw
        assert callable(coleslaw.COMMANDS["voice"])
    ...
    def test_handle_slash_voice_sentinel(self):
        ...
        original_voice = coleslaw.COMMANDS.get("voice")
        try:
            with patch.object(coleslaw, "cmd_voice", return_value=sentinel):
                # Re-bind in COMMANDS so the patch is seen
                coleslaw.COMMANDS["voice"] = coleslaw.cmd_voice
                result = coleslaw.handle_slash("/voice", object(), {})
                assert result == sentinel
        finally:
            if original_voice is not None:
                coleslaw.COMMANDS["voice"] = original_voice
    ...
    def test_voice_status_no_crash(self, capsys):
        ...
        # Should not raise
        try:
            coleslaw.cmd_voice("status", object(), {})
        except SystemExit:
            pass
    ...
    def test_voice_lang_set(self, capsys):
        import coleslaw
        coleslaw.cmd_voice("lang zh", object(), {})
        assert coleslaw._voice_language == "zh"
        # Reset
        coleslaw.cmd_voice("lang auto", object(), {})
        assert coleslaw._voice_language == "auto"
