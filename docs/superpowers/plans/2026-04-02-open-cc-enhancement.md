...
**Goal:** Evolve coleslaw into a capable AI coding CLI with context management, pluggable tools, sub-agents, memory, skills, and diff view.
...
- `coleslaw.py` — Add /memory, /skill, /agents slash commands, diff rendering
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/test_tool_registry.py -v`
...
# tool_registry.py
"""
Tool plugin registry for coleslaw.
...
Run: `cd D:/git/open-cc/coleslaw && python -c "from tools import execute_tool; from tool_registry import get_all_tools; print(f'{len(get_all_tools())} tools registered'); print(execute_tool('Read', {'file_path': 'config.py'}, 'auto'))"`
...
## Task 3: Diff View (`tools.py` + `coleslaw.py`)

**Files:**
- Modify: `tools.py` (the `_edit` and `_write` functions)
- Modify: `coleslaw.py` (the `print_tool_end` function)
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/test_diff_view.py -v`
...
- [ ] **Step 6: Add diff rendering to coleslaw.py**
...
Also ensure the color dict `C` (line 60-70 of coleslaw.py) includes the needed keys. Check and add if missing:
...
Run: `cd D:/git/open-cc/coleslaw && python -c "
...
```bash
cd D:/git/open-cc/coleslaw
git add tools.py coleslaw.py tests/test_diff_view.py
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/test_compaction.py -v`
...
# compaction.py
"""
Context window management for coleslaw.
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/test_memory.py -v`
...
# memory.py
"""
File-based memory system for coleslaw.
...
MEMORY_DIR = Path.home() / ".coleslaw" / "memory"
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/test_subagent.py -v`
...
# subagent.py
"""
Sub-agent lifecycle management for coleslaw.
...
- Modify: `coleslaw.py` (skill dispatch in REPL)
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/test_skills.py -v`
...
# skills.py
"""
Skill loading and execution for coleslaw.
...
SKILL_PATHS = [
    Path.cwd() / ".coleslaw" / "skills",
    Path.home() / ".coleslaw" / "skills",
]
...
- [ ] **Step 5: Integrate skills into coleslaw.py REPL**

In `coleslaw.py`, add import near the top:
...
def cmd_skills(_args, _state, _config):
    skills = load_skills()
    if not skills:
        info("No skills found. Place .md files in ~/.coleslaw/skills/")
        return True
    for s in skills:
...
```bash
cd D:/git/open-cc/coleslaw
git add skills.py tests/test_skills.py coleslaw.py
...
- Modify: `coleslaw.py` (add /memory, /agents commands)
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/ -v`
...
Run: `cd D:/git/open-cc/coleslaw && python -m pytest tests/ -v --tb=short`
