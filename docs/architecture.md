...
This document is for developers who want to understand, modify, or extend coleslaw.
...
User Input
    │
    ▼
coleslaw.py  ── REPL, slash commands, rendering
    │
...
File-based memory system stored in `~/.coleslaw/memory/`.
...
**Search order:** Project-level (`./.coleslaw/skills/`) overrides user-level
(`~/.coleslaw/skills/`) when skill names collide.
...
Defaults stored in `~/.coleslaw/config.json`. Key settings:
...
11. coleslaw.py renders diff with ANSI colors (red/green)
