# ColeSlaw — Update Notes

This document describes three major feature additions to coleslaw:
...
- `~/.coleslaw/agents/<name>.md` (user-level)
- `.coleslaw/agents/<name>.md` (project-level, takes priority)
...
4. `context.py` system prompt template lists Agent, SendMessage, etc. under
   `## Multi-Agent`.
5. `coleslaw.py` `/agents` command calls `get_agent_manager().list_tasks()`
...
| `coleslaw.py` | `/agents` command; background notification; `_tool_desc()` |
...
`~/.coleslaw/memory/`) and **project** (per-repo, `.coleslaw/memory/`).
...
.coleslaw/memory/   ← project-local (relative to cwd)
  MEMORY.md
  ...
```
...
4. `memory.py` (top-level) is now a shim:
   ...
   ```
5. `coleslaw.py` `/memory` command uses `scan_all_memories()` to display a
...
| `coleslaw.py` | `/memory` command uses `scan_all_memories()` |
...
Place `.md` files in:
- `~/.coleslaw/skills/<name>.md` (user-level)
- `.coleslaw/skills/<name>.md` (project-level, takes priority)
...
```
builtin  <  user (~/.coleslaw/skills/)  <  project (.coleslaw/skills/)
```
...
| `coleslaw.py` | `cmd_skills`, `handle_slash`, REPL loop updated |
...
Create `~/.coleslaw/agents/myagent.md`:
```markdown
---
name: myagent
...
`~/.coleslaw/memory/my_note.md` with frontmatter:
```markdown
---
name: my note
...
Create `~/.coleslaw/skills/myskill.md` (user-level) or
`.coleslaw/skills/myskill.md` (project-level):
```markdown
---
name: myskill
...
redirect file system paths to `tmp_path` so no real `~/.coleslaw/`
