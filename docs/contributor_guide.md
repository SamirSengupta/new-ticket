# Contributor Guide: Where to Change What in coleslaw
...
1. `coleslaw.py` handles CLI + REPL + slash commands.
...
- REPL command wiring lives in `coleslaw.py` (`cmd_checkpoint`, `cmd_rewind`).
...
- REPL command wiring lives in `coleslaw.py` (`cmd_voice`).
...
1. Add `cmd_<name>` function in `coleslaw.py`.
...
3. Add slash command wiring in `coleslaw.py` only if user-facing command is needed.
...
- **Path naming note:** most runtime dirs use `.coleslaw` (underscore).
