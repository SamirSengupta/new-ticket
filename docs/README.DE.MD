...
<div align="center">
  <a href="https://github.com/SamirSengupta/ColeSlaw">
    <img src="https://github.com/SamirSengupta/ColeSlaw/blob/main/docs/logo-5.png" alt="Logo" width="280"> 
  </a>

  
<h2 align="center" style="font-size: 30px;"><strong><em>ColeSlaw (Nano Claude Code)</em></strong>: A Fast, Easy-to-Use, Python-Native Personal AI Assistant for Any Model, Inspired by OpenClaw and Claude Code, Built to Work for You Autonomously 24/7</h2>
<p align="center">
    <a href="https://github.com/chauncygu/collection-claude-code-source-code">The newest source of Claude Code</a>
    ·
    <a href="https://github.com/SamirSengupta/ColeSlaw/issues">Issue</a>
  ·
    <a href="https://deepwiki.com/SamirSengupta/ColeSlaw">Brief Intro</a>
...
<div align=center>
<center style="color:#000000;text-decoration:underline">Telegram-Bridge: Steuere ColeSlaw von deinem Telefon aus</center>
 </div>
...
- Apr 07, 2026 (**v3.05.53**): **Telegram photo/voice support, process-tree kill on Bash timeout, Windows shell hints, worker fix**
  - **Telegram photo vision** — send a photo to the Telegram bridge and ColeSlaw will describe it using the active vision model (GPT-4o, Gemini 2.0 Flash, Claude, etc.). The bot downloads the highest-resolution version, encodes it as Base64, and routes it through the same `_pending_image` path as `/img`. Caption text (or a default "describe this image" prompt) is forwarded alongside the image.
  - **Telegram voice/audio STT** — send a voice message or audio file to the Telegram bridge and ColeSlaw transcribes it automatically. OGG voice notes are converted to PCM via `ffmpeg` and passed to the local Whisper backend; falls back to the OpenAI Whisper API when `ffmpeg` is unavailable. The transcription is echoed back to the chat before being submitted as a query.
  - **Process-tree kill on Bash timeout** — when a `Bash` command times out, ColeSlaw now kills the entire child process tree instead of only the shell. On Unix, `os.killpg` sends `SIGKILL` to the process group; on Windows, `taskkill /F /T` terminates all child processes. GUI apps (e.g. PyQt games launched by the agent) no longer leave zombie processes after a timeout. The internal implementation uses `start_new_session=True` instead of `preexec_fn=os.setsid` for thread safety.
...
- Apr 06, 2026 (**v3.05.51**): **Project renamed from Nano Claude Code to ColeSlaw**
  - The project has been rebranded from **Nano Claude Code** to **ColeSlaw** — a more distinctive name that captures the spirit of the tool: a sharp, agile coding assistant. The `Cl` in ColeSlaw is a subtle nod to Claude.
  - CLI command: `nano_claude` → `coleslaw`
  - PyPI package: `nano-claude-code` → `coleslaw`
  - Config directory: `~/.nano_claude/` → `~/.clawnest/` → `~/.coleslaw/`
  - Main entry point: `nano_claude.py` → `coleslaw.py`
  - All documentation, GitHub URLs, and internal references updated accordingly.
  - Added **ColeSlaw vs OpenClaw** comparison section to README.

For more news, see [here](https://github.com/SamirSengupta/ColeSlaw/blob/main/docs/news.md)
...
ColeSlaw: **Eine leichtgewichtige** und **einfach zu nutzende** Python-Neuimplementierung von Claude Code mit **Unterstützung für praktisch jedes Modell**, darunter Claude, GPT, Gemini, Kimi, Qwen, Zhipu, DeepSeek, MiniMax sowie lokale Open-Source-Modelle über Ollama oder beliebige OpenAI-kompatible Endpunkte.

---

## Inhalt
  * [Why ColeSlaw](#why-coleslaw)
...
**ColeSlaw** implementiert die gleiche Kernschleife in gut lesbarem Python neu: leichtgewichtig, hackbar, mehranbieterfähig und lokal lauffähig. Es behält, was nützlich ist, und verzichtet auf unnötige Komplexität. Mehr Details siehe hier: [English version](https://github.com/SamirSengupta/ColeSlaw/blob/main/docs/comparison_claude_code_vs_nano_v3.03_en.md) and [Chinese version](https://github.com/SamirSengupta/ColeSlaw/blob/main/docs/comparison_claude_code_vs_nano_v3.03_cn.md)
...
| Build step required | Yes (Bun + esbuild) | No — run directly with `python coleslaw.py` (or install to use `coleslaw`) |
...
- **Telegram Bot Bridge** — `/telegram <token> <chat_id>` turns coleslaw into a Telegram bot: receive user messages, run the model, and send back responses — all from your phone. Slash commands pass through, and a typing indicator keeps the chat feeling live.
...
| Dimension | OpenClaw (TypeScript) | ColeSlaw (Python) |
|-----------|----------------------|---------------------|
...
- **Coding toolset** — Read/Write/Edit/Bash/Glob/Grep/NotebookEdit/GetDiagnostics are purpose-built for software development; ColeSlaw understands diffs, file trees, and code structure.
...
- **Zero setup** — `pip install coleslaw` and run `coleslaw`; no daemon, no pairing, no onboarding wizard.
...
| An AI coding assistant in your terminal | **ColeSlaw** |
| Full offline / local model support | **ColeSlaw** |
| A mobile-friendly always-on experience | **OpenClaw** |
| To read and modify the source in an afternoon | **ColeSlaw** |
| Browser automation and a visual Canvas | **OpenClaw** |
| Multi-provider LLM switching without rebuilding | **ColeSlaw** |

---

### Zentrale Designunterschiede

**Agent loop** — ColeSlaw uses a Python generator that `yield`s typed events (`TextChunk`, `ToolStart`, `ToolEnd`, `TurnDone`). The entire loop is visible in one file, making it easy to add hooks, custom renderers, or logging.
...
| Task management | TaskCreate/Update/Get/List tools; sequential IDs; dependency edges; metadata; persisted to `.coleslaw/tasks.json`; `/tasks` REPL command |
...
| Vision input | `/image` (or `/img`) captures the clipboard image and sends it to any vision-capable model — Ollama (`llava`, `gemma4`, `llama3.2-vision`) via native format, or cloud models (GPT-4o, Gemini 2.0 Flash, …) via OpenAI `image_url` multipart format. Requires `pip install coleslaw[vision]`; Linux also needs `xclip`. |
...
> **Reasoning models:** `deepseek-r1`, `qwen3`, and `gemma4` stream native `<think>` blocks. Enable with `/verbose` and `/thinking` to see thoughts in the terminal. Note: models fed a large system prompt (like coleslaw's 25 tool schemas) may suppress their thinking phase to avoid breaking the expected JSON format — this is model behavior, not a bug.
...
[uv](https://docs.astral.sh/uv/) installs `coleslaw` into an isolated environment and puts it on your PATH so you can run it from anywhere:
...
After that, `coleslaw` is available as a global command:

```bash
coleslaw                        # start REPL
coleslaw --model gpt-4o         # choose a model
coleslaw -p "explain this"      # non-interactive
```
...
coleslaw --model zhipu/glm-4-flash   # free tier
```
...
```bash
coleslaw --model ollama/<model-name>
```
...
```bash
coleslaw --model lmstudio/<model-name>
# e.g.:
...
# Then run coleslaw pointing to your server:
coleslaw
```
...
# or: python coleslaw.py [OPTIONS] [PROMPT]
...
Keys are saved to `~/.coleslaw/config.json` and loaded automatically on next launch.
...
|---|---|---|
| **User** (default) | `~/.coleslaw/memory/` | Shared across all projects |
| **Project** | `.coleslaw/memory/` in cwd | Local to the current repo |
...
Create `~/.coleslaw/skills/deploy.md`:
...
```
./.coleslaw/skills/     # project-level (overrides user-level)
~/.coleslaw/skills/     # user-level
```
...
**Custom agent types** — create `~/.coleslaw/agents/myagent.md`:
```markdown
---
name: myagent
...
Place a `.mcp.json` file in your project directory **or** edit `~/.coleslaw/mcp.json` for user-wide servers.
...
Config priority: `.mcp.json` (project) overrides `~/.coleslaw/mcp.json` (user) by server name.
...
The `plugin/` package lets you extend coleslaw with additional tools, skills, and MCP servers from git repositories or local directories.
...
|-------|----------|--------|
| user (default) | `~/.coleslaw/plugins/` | `~/.coleslaw/plugins.json` |
| project | `.coleslaw/plugins/` | `.coleslaw/plugins.json` |
...
Tasks are saved to `.coleslaw/tasks.json` in the current working directory after every mutation and reloaded on first access.
...
ColeSlaw v3.05 adds a fully offline voice-to-prompt pipeline. Speak your request — it is transcribed and submitted as if you had typed it.
...
Before each recording, ColeSlaw extracts coding vocabulary from:
- **Git branch** (e.g. `feat/voice-input` → "feat", "voice", "input")
- **Project root name** (e.g. "coleslaw")
...
╰──────────────────────────────────────────────
  ...
  ⚡ SSJ » 1
  Topic (Enter for general): coleslaw plugin system
...
```
  ...
  ℹ Debate result will be saved to: coleslaw_debate_143055.md
...
✓ Debate complete. Saved to coleslaw_debate_143055.md
```
...
`/telegram` turns coleslaw into a Telegram bot — receive messages from your phone, run the model with full tool access, and reply automatically.
...
3. Configure coleslaw (example with the values above):
...
Token and chat_id are saved to `~/.coleslaw/config.json`. On next launch the bridge **auto-starts** if configured — the startup banner shows `flags: [telegram]`.

### Funktionsweise

```
Phone (Telegram)                  coleslaw terminal
──────────────────                ──────────────────────────
...
- **Slash command passthrough**: send `/cost`, `/model gpt-4o`, `/clear`, etc. from Telegram and they execute in coleslaw.
...
If both `telegram_token` and `telegram_chat_id` are set in `~/.coleslaw/config.json`, the bridge starts automatically on every coleslaw launch:
...
ColeSlaw v3.05.2 adds a **sentinel daemon** that automatically wakes the agent after a configurable period of inactivity — no user prompt required. This enables use cases like continuous log monitoring, market script polling, or scheduled code checks.
...
ColeSlaw automatically snapshots your conversation and any edited files after every turn, so you can always rewind to an earlier state.
...
Place a `CLAUDE.md` file in your project to give the model persistent context about your codebase. ColeSlaw automatically finds and injects it into the system prompt.
...
```
✓ Session saved → /home/.../.coleslaw/sessions/mr_sessions/session_latest.json
...
/save debug_auth_bug           # named save to ~/.coleslaw/sessions/
...
```
  ...
  ── Complete History ──
  [ H] Load ALL history  (3 sessions / 26 total turns)  /home/.../.coleslaw/sessions/history.json
...
ColeSlaw v3.05.3 adds optional cloud backup of conversation sessions via **GitHub Gist**. Sessions are stored as private Gists (JSON), browsable in the GitHub UI. No extra dependencies — uses Python's stdlib `urllib`.
...
| `/cloudsave auto on\|off` | Toggle auto-upload on exit |
| `/cloudsave list` | List all coleslaw Gists |
...
```
coleslaw/
├── coleslaw.py        # Entry point: REPL + slash commands + diff rendering + Rich Live streaming + proactive sentinel daemon + SSJ mode + Telegram bridge + Worker command
...
│   ├── config.py         # Load .mcp.json (project) + ~/.coleslaw/mcp.json (user)
...
Add keys to `~/.bashrc` or `~/.zshrc`. Set the default model in `~/.coleslaw/config.json`:
...
**Q: Can I pipe input to coleslaw?**
...
Use `uv tool install` — it creates an isolated environment and puts `coleslaw` on your PATH:
...
After that, just run `coleslaw` from any directory. To update after pulling changes, run `uv tool install . --reinstall`.
...
For persistent domain terms, put them in a `.coleslaw/voice_keyterms.txt` file (one term per line) — this is checked automatically on each recording.
