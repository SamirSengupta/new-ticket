"""Configuration management for ColeSlaw (multi-provider)."""
import os
import json
import shutil
from pathlib import Path

CONFIG_DIR        = Path.home() / ".coleslaw"
LEGACY_CONFIG_DIR = Path.home() / ".cheetahclaws"
...
def _migrate_legacy_home_dir() -> None:
    """Best-effort one-time migration from legacy config dir to ~/.coleslaw."""
    if CONFIG_DIR.exists() or not LEGACY_CONFIG_DIR.exists():
        return
    try:
        shutil.copytree(LEGACY_CONFIG_DIR, CONFIG_DIR)
    except Exception:
        # Migration should never block startup.
        pass
