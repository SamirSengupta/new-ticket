"""Tmux integration tools for ColeSlaw.
...
def _find_tmux() -> str | None:
    ...
    if sys.platform == "win32":
        ...
        custom = os.environ.get("COLESLAW_PSMUX_PATH")
        if custom:
...
def _tmux_new_session(params: dict, config: dict) -> str:
    name = _safe(params.get("session_name", "coleslaw"))
...
TMUX_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "session_name": {"type": "string", "description": "Session name (default: coleslaw)"},
