"""
Cloud sync for coleslaw sessions via GitHub Gist.
...
Config keys (stored in ~/.coleslaw/config.json):
...
GIST_TAG = "[coleslaw]"
LEGACY_GIST_TAGS = ("[cheetahclaws]",)
ALL_GIST_TAGS = (GIST_TAG,) + LEGACY_GIST_TAGS
...
def strip_session_tag(description: str) -> str:
    """Trim known coleslaw/legacy tags from a Gist description."""
    for tag in ALL_GIST_TAGS:
        if description.startswith(tag):
            return description[len(tag):].strip()
...
def upload_session(
    ...
    filename = f"coleslaw_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def list_sessions(token: str, max_results: int = 20) -> tuple[list[dict], str | None]:
    """
    List Gists tagged as coleslaw sessions (including legacy tags).
    ...
    sessions = [
        ...
        for g in result
        if any(g.get("description", "").startswith(tag) for tag in ALL_GIST_TAGS)
    ]
