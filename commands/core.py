"""
commands/core.py — Core utility commands for ColeSlaw.
...
# VERSION is imported lazily from coleslaw to avoid circular imports
_VERSION_STR = ""
...
def _get_version() -> str:
    ...
    if not _VERSION_STR:
        try:
            import importlib
            cc = importlib.import_module("coleslaw")
...
def cmd_help(_args: str, _state, config) -> bool:
    try:
        import coleslaw
        print(coleslaw.__doc__)
    except Exception:
        info("ColeSlaw — type /model, /save, /load, /history, /context, /exit for commands.")
    return True
...
def cmd_image(args: str, state, config) -> Union[bool, tuple]:
    ...
    try:
        ...
        err("Pillow is required for /image. Install with: pip install coleslaw[vision]")
