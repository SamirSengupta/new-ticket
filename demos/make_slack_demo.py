#!/usr/bin/env python3
"""
Generate animated GIF demo of coleslaw Slack Bridge.
...
def draw_slack(img, messages):
    ...
    d.text((main_x + 24, py + 8), "coleslaw", font=FONT_B, fill=(30, 30, 30))
    d.text((main_x + 12, py + 26), "coleslaw bot channel", font=FONT_XS, fill=(100, 100, 100))
    ...
    d.text((main_x + 20, input_y + 12), "Message #coleslaw", font=FONT_SM, fill=(180, 180, 180))
...
def prompt_line(text="", cursor=False):
    cur = "█" if cursor else ""
    return [seg("[coleslaw] ", SUBTEXT), seg("» ", CYAN, True), seg(text + cur, TEXT)]
...
def build_scenes():
    ...
    slack_init = [
        (BOT_NAME, "🟢 coleslaw is online. Send me a message and I'll process it.", True, False),
    ]
    ...
    resp1_lines = [
        ...
        "",
        "  coleslaw.py   — Main REPL + slash commands",
    ...
    slack_r1 = slack_q1 + [
        (BOT_NAME, "Here are the files: coleslaw.py, agent.py, tools.py, providers.py, config.py …", True, False),
    ]
    ...
    add(q3_base + [
        ...
        tool_ok("Found in coleslaw.py:480"),
        None,
        tool_line("⚙", "Read", "coleslaw.py:480-550", CYAN),
    ...
    slack_stopped = slack_stop + [(BOT_NAME, "🔴 coleslaw bridge stopped.", True, False)]
