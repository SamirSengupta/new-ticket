...
Storage layout:
  user scope    : ~/.coleslaw/memory/<slug>.md
  project scope : .coleslaw/memory/<slug>.md  (relative to cwd)
...
USER_MEMORY_DIR = Path.home() / ".coleslaw" / "memory"
...
def get_project_memory_dir() -> Path:
    ...
    return Path.cwd() / ".coleslaw" / "memory"
...
def get_memory_dir(scope: str = "user") -> Path:
    ...
    Args:
        scope: "user" (global ~/.coleslaw/memory) or
               "project" (.coleslaw/memory relative to cwd)
    """
    if scope == "project":
